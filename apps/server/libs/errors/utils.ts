import type { ErrorCode } from "@acme/error";
import type { Context } from "hono";
import { HTTPException } from "hono/http-exception";
import { ZodError, z } from "zod";

import { ErrorCodeEnum, SchemaError, statusToCode } from "@acme/error";
    ...
    return c.json<ErrorSchema>(
      {
        ...
        docs: "https://docs.acme.ai/api-references/errors/code/BAD_REQUEST",
      },
      { status: 400 }
    );
  }
    ...
    return c.json<ErrorSchema>(
      {
        code: code,
        message: err.message,
        docs: `https://docs.acme.ai/api-references/errors/code/${code}`,
      },
      { status: err.status }
    );
  }
  return c.json<ErrorSchema>(
    {
      ...
      docs: "https://docs.acme.ai/api-references/errors/code/INTERNAL_SERVER_ERROR",
    },
...
export function createErrorSchema(code: ErrorCode) {
  return z.object({
    ...
    docs: z.string().openapi({
      ...
      example: `https://docs.acme.ai/api-references/errors/code/${code}`,
    }),
  });
}
